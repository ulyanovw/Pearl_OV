...
diffie_hellman_update(){
	...
	if [[ ${dhpemacc} == "y" ]]; then
		...
		echo "   3) 4096 байт"
		echo "   3) 8192 байт"
		until [[ $DH_UPDATE_KEY_SIZE_CHOICE =~ ^[1-3]$ ]]; do
			read -rp "Размер ключа Диффи-Хеллмана [1-3]: " -e -i 1 DH_UPDATE_KEY_SIZE_CHOICE
		done
...
Additional(){
 ...
 ${Blue}7.${Font_color_suffix} ${Yellow}Установка утилиты Speedtest${Font_color_suffix}"
  read -e -p "Выберите пункт(ENTER для выхода): " choose
