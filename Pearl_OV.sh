...
delete_OV_instance() {
    ...
    for instance in "${selected_instances[@]}"; do
        ...
        if [[ "$confirm_delete" =~ ^[yY]$ ]]; then
            sudo systemctl disable --now "iptables-openvpn-server-$proto-$prt.service"
            sudo systemctl disable --now "openvpn-server-$proto-$prt.service"
            sudo rm "/etc/systemd/system/iptables-openvpn-server-$proto-$prt.service"
            sudo rm "/etc/systemd/system/openvpn-server-$proto-$prt.service"
            sudo rm "/etc/iptables/add-openvpn-server-$proto-$prt.sh"
            sudo rm "/etc/iptables/rm-openvpn-server-$proto-$prt.sh"
            sudo systemctl daemon-reload
...
OVPN_install(){
    ...
    if [[ "$os" = "debian" || "$os" = "ubuntu" ]]; then
        apt-get update
        apt-get install -y jq at openvpn net-tools openssl ca-certificates 
