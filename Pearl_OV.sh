...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	new_client
        clear
	curl -s -X POST https://api.telegram.org/bot"$bot_api"/sendMessage -d chat_id="$tgid" -d text="OpenVPN был развёрнут с использованием Pearl_OV на этом сервере: $public_ip"
