...
Autofeedbackmenu(){
  echo -e "Меню управления автоматизациями: "
  echo -e " ${Blue}1.${Font_color_suffix}${Yellow} Загрузить скрипты для автоматизации${Font_color_suffix}
 			${Blue}2.${Font_color_suffix} ${Yellow}Управление автоматизациями${Font_color_suffix}"
   read -e -p "Выберите пункт(ENTER для выхода): " choose
	[[ -z "${choose}" ]] && echo -e "${Error}Отмена..." && exit 1
	if [[ ${choose} == "1" ]]; then
		AutofeedbackSHdownloader
	elif [[ ${choose} == "2" ]]; then
		FeedbackMenu
	else
...
AutofeedbackSHdownloader(){
    if [ -e /root/Autofeedback/]
    then
        clear
        echo "Все необходимые скрипты для автоматизаций уже загружены!"
        exit 1
    else
        ...
        clear
        cd
        echo "Загрузка необходимых скриптов завершена!"
    fi
}
ip_list(){
  read -p "Введите ключевое слово или дату в формате DD-MM(Или просто нажмите ENTER): " dealer
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	apt install at
	clear
	echo 'Добро пожаловать в установщик OVpro (модификация для NPB)!'
	...
	- $ip is a private IP address, the server must be behind NAT+++-
	...
	echo "   2) 3072 байт"
	echo "   3) 4096 байт"
	read -p "Размер ключа ДХ [1-3]: " DH_KEY_SIZE_CHOICE
	...
	echo "   3) 3072 байт"
	echo "   4) 4096 байт"
	read -p "Размер ключа RSA [1-4]: " RSA_KEY_SIZE_CHOICE
